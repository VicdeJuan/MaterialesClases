...
\ifverbose
Los posibles puntos en los que la función puede tener una asíntota vertical son aquellos en los que se anula el denominador o los que hacen 0 el argumento del logaritmo. 
%
\[\sagestr{solveDen0(f)} \]
\fi
...
\begin{figure}
    \begin{center}
        \sageplot{_myplot(f,AH=uniq(AH),AV=uniq(AV),AO=AO,intervalos_dominio = realSetDomainVar,listmin=listmin,listmax=listmax,inflexionlist=[])}
    \end{center}
...
def _myplot(f,AV,AH,AO,intervalos_dominio,listmin,listmax,inflexionlist):
 ...
 for i in xrange(len(AV)):
  lab="x="+str(AV[i])
  v=AV[i]
  P+=line([(v,ymin),(v,ymax)],legend_label=lab,color='blue',linestyle='-',thickness=1)
  j=i
 ...
 if AO != []:
  for i in xrange(len(AO)):
   lab=str(AO[i])
   P+=plot(AO[i],color='purple',legend_label="y="+lab,xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax,thickness=2,linestyle='--')
 p=[a for a in listmin]
 [p.append(b) for b in listmax]
 for i in p:
   P+=circle((i,f(i)),0.1,facecolor='black',fill=True)
 return P
\end{sagesilent}
