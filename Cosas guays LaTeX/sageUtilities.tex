...
def ptsDiscontinuidad(f):
    ...
    print("log : funcion ptsDiscontinuidad: dominio: "+str(intervalos_dominio))
    retval = []
    for ip in list(intervalos_dominio.complement()):
        if ip.is_point():
            retval.append(ip.lower())
    print("log : funcion ptsDiscontinuidad: valor de retorno: " + str(retval))
    return retval
...
def asintotesV(f,den,AV):
    print("log: funcion asintotesV: asíntotas recibidas: "+str(AV))
    _emptylist(AV)
    print("log: funcion asintotesV: asíntotas vaciadas: "+str(AV))
    ...
    if (hasLog > 0):
        ...
        r+= "Soluciones: "
        intervalos_dominio = dominion(f,false)
        print("log: funcion asintotesV: dominio: " + str(intervalos_dominio))
        NoCandidates=[]
        for i in log0:
...
def solveDerivadaNula(f,sols,onlyReal):
    print("log: solveDerivadaNula" +  str(f))
    intervalos_dominio = dominion(f,false)
    ...
    for r in sols:
        ...
        if not i in intervalos_dominio:
...
def _getRealroots(f):
 print("log: _getRealroots de " + str(f))
 __roots = solve(f==0,x,solution_dict=True)
 print("log: \t soluciones: "+str(__roots))
 _roots = []
 for a in __roots:
    print("log:\t estudiando "+str(a[x]))
    if type(a[x]) == type(x+1):
        print("log: \t es expresión, calculamos solución numérica..."+str(a) + " [x]-> "+str(a[x]))        
        sol = find_root(f,-Infinity,Infinity)
        print("log: \t encontrada solucion: "+str(sol))
        _roots.append(sol)
    else:
        print("log: \t no es expresión ")
        if imag(a[x]) == 0:
            _roots.append(a[x].full_simplify())
 print("log: \t soluciones to return: " + str(_roots))
 return _roots
...
## IDEAS PARA MEJORAR. Poner los "&" al final, para poder poner \\hline entre las filas de las tablas.
 ...
 print("log: _f_sign_monot_tabla: \t f:"+str(f) + "\n\t curvatura: "+str(curvatura))
 ...
 if curvatura:
    ...
    print("log: \t solo monotonia" )
    __roots = _getRealroots(df) 
    print("log: raíces df: \t "+str(__roots))
    for a in __roots:
        if a in intervalos_dominio:
            recta.append(a)
    for a in ptsDiscontinuidad(f=f):
 ...
 print("log: _f_sign_monot_tabla: recta: "+str(recta))
 ...
 print("log: _f_sign_monot_tabla: comenzamos a construir la tabla")
...
def __simpleInecToInterval(s):
    print("log: __simpleInecToInterval de s: " + str(s))
    ineqOperators = {
        (x<3).operator():"<",
        (x>3).operator():">",
        (x<=3).operator():"<=",
    ...
    print("log: \t left,right,operator"+str(left)+","+str(right)+","+str(operator))
    if left.is_numeric() or left.is_infinity():
        print("log: \t left: "+str(left))
        num = left
    elif right.is_numeric() or right.is_infinity():
        print("log: \t right: "+str(right))
        num = right
    else:
        num = "Error grave"
...
def __getIntervalsFromIneq(tosolve,intervals):
    print("log: __getIntervalsFromIneq")
    print("\t tosolve: "+str(tosolve))
    print("\t intervals: "+str(intervals))
    solutions = tosolve.solve(x)
    print("\t solucions: "+str(solutions))

    __aux = []
    for i in solutions:
        if type(i) == type(x+1):
            print("log: \t is expression")
            __aux.append(RealSet(i))
        else:
            print("log: \t is not expression")
            __aux.append(RealSet(i[0]))

    print("log: \t __aux - intervals: "+str(__aux))
    ...
    for i in solutions:
        if type(i) == type(x+1):
            ...
            print("log: terminado __simpleInecToInterval")
        else:
            print("log: \t is not expression")
            __aux.append(__simpleInecToInterval(i[0]))
            print("log: terminado __simpleInecToInterval")

    print("log: \t __aux - sols: "+str(__aux)) 
...
def dominion(f,maketext):
    ...
    if not denominator.is_constant():
        ...
        __solutions = [a[x] for a in solve(f.denominator() == 0,x,solution_dict=True,domain='real')]   
    ...
    print("log (funcion dominion): intervalos_dominio " + str(intervalos_dominio))
    if len(intervals) == 0:
    ...
    print("log (funcion dominion): dominio posteignacion" + str(intervalos_dominio))
    
    print("log (funcion dominion) variable global"+str(intervalos_dominio))
    
    if maketext:
        return "D(f) = {"+doms + "} = {" + str(__dominio).replace("oo","\\infty") + "}"
    else:
