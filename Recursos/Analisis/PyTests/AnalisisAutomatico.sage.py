...
# This file was *autogenerated* from the file PyTests/AnalisisAutomatico.sage
from sage.all_cmdline import *   # import sage library

_sage_const_3 = Integer(3); _sage_const_2 = Integer(2); _sage_const_9999999999 = Integer(9999999999); _sage_const_0 = Integer(0); _sage_const_1000000 = Integer(1000000); _sage_const_6 = Integer(6); _sage_const_2500 = Integer(2500); _sage_const_8 = Integer(8); _sage_const_1 = Integer(1); _sage_const_2p0 = RealNumber('2.0'); _sage_const_10 = Integer(10); _sage_const_0p0000001 = RealNumber('0.0000001'); _sage_const_0p2 = RealNumber('0.2')
...
AH=[]
AO=[]
realSetDomainVar = RealSet(_sage_const_0 ,_sage_const_0 )
ptsCrit=[]
recta=[]
min=[]
max=[]

epsilon = _sage_const_0p0000001 
...
def random_between(j,k):
 ...
 if (j==-Infinity):
  return int(k-_sage_const_2 )

 if (k==Infinity):
  return int(j+_sage_const_2 )

 if (j < _sage_const_0  and k>_sage_const_0 ):
  return _sage_const_0 

 if (j < _sage_const_1  and k>_sage_const_1 ):
  return _sage_const_1 


 if (k-j>=_sage_const_2 ):
  return int(random()*(k-j-_sage_const_1 ))+j+_sage_const_1 
 else:
  return round((k+j)/_sage_const_2p0 ,_sage_const_2 )
...
def limit_lateral(funcion,v,lado):
    ...
    if imag(funcion(_test_val)) != _sage_const_0 :
        return None
    else:
...
def getMinFromDomain(interval):
    if len(list(interval)) > _sage_const_0 :
        return interval[_sage_const_0 ].lower()
    else: 
...
def getMaxFromDomain(interval):
    if len(list(interval)) > _sage_const_0 :
        return list(interval)[-_sage_const_1 ].upper()
    else: 
        logging.info("Error:  funcion getMinFromDomain intentando conseguir el minimo de "+str(interval))
        return None
...
def asintotesV(f,den,AV,intervalos_dominio):
    ...
    if (hasLog > _sage_const_0 ):
        _log0 = []
        
        for arg in argsLog:
            [_log0.append(a) for a in solve(arg == _sage_const_0  , x , solution_dict=True)]
            if not arg.denominator().is_constant():
                [_log0.append(a) for a in solve(arg.denominator() == _sage_const_0  , x , solution_dict=True)]                

        logging.info(_log0)
        log0 = [a[x] for a in _log0 if imag(a[x])==_sage_const_0 ]  
        ...
        if (log0 == []):
            r+= "No tiene soluciones reales"
        elif len(NoCandidates) > _sage_const_0 :
            r+= "\\\\De estas soluciones, "
            for a in NoCandidates:
                r+="$x_"+str(a[_sage_const_0 ])+"="+latex(i)+" $;\\quad"                
...
def asintotesHO(f,AH,AO,intervalos_dominio):
    ...
    if _sage_const_9999999999  in intervalos_dominio:
    ...
    if len(infsToCalcAsintotes) == _sage_const_0 :
    ...
    for infty in infsToCalcAsintotes:
        ...
        if (not (abs(ld) == Infinity)): # HORIZONTAL
            ...
            if (abs(m) != Infinity and m != _sage_const_0 ):
                if (Verbose == _sage_const_1 ):
...
def puntosEjeX(func,intervalos_dominio):
    r=""
    __roots = solve(func(x)==_sage_const_0 ,x,solution_dict=True,explicit_solutions=True)
    _roots = [a[x] for a in __roots if imag(a[x])==_sage_const_0 ]
...
def puntosEjeY(func,intervalos_dominio):
    if _sage_const_0  in intervalos_dominio:
        return "$f(0) = " + latex(func(_sage_const_0 )) + "$, con lo que la grafica corta en $\\left(0,"+latex(func(_sage_const_0 )) + "\\right)$ al eje Y."   
    else:
...
def solveDerivadaNula(f,sols,onlyReal,intervalos_dominio):
    ...
    _sols = solve(diff(f(x))==_sage_const_0 ,x,solution_dict=True,explicit_solutions=True)
    if onlyReal:
        sols = [a[x] for a in _sols if imag(a[x])==_sage_const_0 ]
    else: 
        sols=_sols
    ...
    if len(NoCandidates) > _sage_const_0 :
        retval += "\\\\De estas soluciones, "
        for a in NoCandidates:
            retval +="$x_"+str(a[_sage_const_0 ])+"="+latex(a[_sage_const_1 ].full_simplify())+" $;\\quad"
...
def ptsFrontera(f,ptscrit,recta,derivada_too,intervalos_dominio):
    ...
    if derivada_too:
        ___pts = ptsDiscontinuidad(f=diff(f(x),x,_sage_const_1 ))
        if ___pts != []:
    ...
    if len(puntosFrontera) == _sage_const_0 :
        r += " no hay."
...
def intervalos(recta,domain = None):
    ...
    r = ""
    for i in xrange(len(recta)-_sage_const_1 ):
        if random_between(recta[i],recta[i+_sage_const_1 ]) in domain:
            r += "$\\left("+latex(recta[i])+","+latex(recta[i+_sage_const_1 ])+"\\right)$;\\quad"  
        else:
            logging.info("funcion intervalos: el intervalo (" + str(recta[i]) + "," + str(recta[i+_sage_const_1 ]) + ") no esta en el dominio.")
    return r
...
def estudiarSigno(f,intervalos_dominio):
    ...
    for i in xrange(len(recta)-_sage_const_1 ):
        rval = random_between(recta[i],recta[i+_sage_const_1 ])
        retval += "\\item $\\left("+latex(recta[i])+","+latex(recta[i+_sage_const_1 ])+"\\right)$:"
        if (Verbose == _sage_const_1 ):
            retval += " Tomamos, por ejemplo, $f("+latex(rval)+") = "+latex(round(f(rval),_sage_const_3 ))+"...$  y miramos su signo:" 
        else:
            retval += "$f("+latex(rval)+") = "+latex(round(f(rval),_sage_const_3 )) +"$"
        actual=sign(f(rval))
        if actual==_sage_const_1 : 
            if (Verbose == _sage_const_1 ):
                retval += "Positivo"  
            else:
                retval += "+"
...
def _getRealroots(f):
 ...
 __roots = solve(f==_sage_const_0 ,x,solution_dict=True)
...
def _f_sign_monot(recta,f,min,max,curvatura,intervalos_dominio):
 _emptylist(min)
 _emptylist(max)
 df=diff(f,x,_sage_const_1 )
 ddf = diff(df,x,_sage_const_1 )
 ...
 for i in xrange(len(recta)-_sage_const_1 ):
  ...
  retval += "\\item $\\left("+latex(recta[i])+","+latex(recta[i+_sage_const_1 ])+"\\right)$:"
  if (Verbose == _sage_const_1 ):
   retval += " Tomamos, por ejemplo, "+fun+"($"+latex(rval)+") = "+latex(round(g(rval),_sage_const_3 ))+"...$  y miramos su signo:" 
  else:
   retval += fun + "($"+latex(rval)+") = "+latex(round(g(rval),_sage_const_3 )) +"$"
  prev=actual
  actual=sign(g(rval))
  if actual==_sage_const_1 : 
   if (Verbose == _sage_const_1 ):
...
## VER f_sign_monot sin tabla.
 ...
 __label = "estudio" + str(random_between(_sage_const_1 ,_sage_const_1000000 )) + str(random_between(_sage_const_1 ,_sage_const_1000000 )) + str(random_between(_sage_const_1 ,_sage_const_1000000 ))
 retval += "Ver tabla \\ref{"+__label+"}, con el estudio de "+fun+"(x)"


 prev=_sage_const_0 
 actual=_sage_const_1 
 ...
 intervalos_a_excluir = _sage_const_0 
 for i in xrange(len(recta)-_sage_const_1 ):
    if not random_between(recta[i],recta[i+_sage_const_1 ]) in intervalos_dominio:
        intervalos_a_excluir+=_sage_const_1 
 ...
 for i in xrange(len(recta)-_sage_const_1 ):
  if not random_between(recta[i],recta[i+_sage_const_1 ]) in intervalos_dominio:
        continue   
  retval += "& $\\left("+latex(recta[i])+","+latex(recta[i+_sage_const_1 ])+"\\right)$"
 ...
 for i in xrange(len(recta)-_sage_const_1 ):
  logging.info("\t\t _f_sign_monot_tabla: estudiando intervalo: " + str([recta[i],recta[i+_sage_const_1 ]])) 
  rval=random_between(recta[i],recta[i+_sage_const_1 ])
  if not rval in intervalos_dominio:
    continue
  retval += "&"+fun + "($"+latex(rval)+") = "+latex(round(g(rval),_sage_const_3 )) +"$"
  if sign(g(rval)) == _sage_const_1 :
    if curvatura:
  ...
  if not i == _sage_const_0 :
      if prev!=actual and actual == -_sage_const_1  and toadd in intervalos_dominio:
        max.append(toadd)
      if prev!=actual and actual == _sage_const_1  and toadd in intervalos_dominio:
        min.append(toadd)
 ...
 if not curvatura:
    ...
    elif len(max) == _sage_const_1 :
        retval += "El punto $x=" + latex(max[_sage_const_0 ]) + "$ es un maximo de la funcion.\\\\" 
    else:
        retval += "Los puntos $" + printlist(max) + "$ son maximos de la funcion.\\\\" 
    retval += "\\item"
    if min == []:
        retval += "La funcion no tiene ningun minimo."
    elif len(min) == _sage_const_1 :
        retval += "El punto $x=" + latex(min[_sage_const_0 ]) + "$ es un minimo de la funcion.\\\\"
    else:
...
def getRadicandos(f):
    return flatten(__hasRt_aux(f,_sage_const_0 ))
...
def getArgsLog(f):
    return flatten(__hasLog_aux(f,_sage_const_0 ))
...
def dominion(f,maketext,realSetDomainArg):
    intervalos_dominio = RealSet(_sage_const_0 ,_sage_const_0 )
    ...
    if hasLog:
        ...
        _to_domain = [__getIntervalsFromIneq(argument>_sage_const_0 ,intervals) for argument in argsLog]
...
def _myplot(f,AV,AH,AO,intervalos_dominio):
 ...
 P=plot(f(x),xmin=xmin,xmax=xmax,ymin=ymin,ymax=ymax,figsize=_sage_const_6 ,plot_points=_sage_const_2500 ,legend_label="f(x)", axes_labels=['$x$','$y$'],exclude=discont,color='green')
 ...
 for i in xrange(len(discont)):
  ...
  P+=circle((x0,ld),_sage_const_0p2 ,facecolor='white',fill=True)
 if AO != []:
...
print(latex(diff(f(x),x,_sage_const_1 ).full_simplify()))
