#coding: utf-8


# This file was *autogenerated* from the file generatorOfPolynomials.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_25 = Integer(25); _sage_const_5 = Integer(5); _sage_const_2 = Integer(2); _sage_const_4 = Integer(4); _sage_const_1p0 = RealNumber('1.0'); _sage_const_7 = Integer(7); _sage_const_9 = Integer(9); _sage_const_3 = Integer(3); _sage_const_6 = Integer(6); _sage_const_1p2 = RealNumber('1.2'); _sage_const_10 = Integer(10); _sage_const_15 = Integer(15); _sage_const_20 = Integer(20)
import re
var('x')
var('y')
var('z')
var('a')
var('b')
var('c')
var('d')
var('e')
var('s')
var('t')
var('g')

__letras = [x,y,z,a,b,c,d,e,s,t,g]
#set_random_seed(0)

def _latex(ss):
    s=str(ss)
    for gs in map(list,re.findall("([0-9]+)/([0-9]+)",s)):
        s=re.sub(gs[_sage_const_0 ]+"/"+gs[_sage_const_1 ],"\\\\frac{"+gs[_sage_const_0 ]+"}{"+gs[_sage_const_1 ]+"}",s)
    s=s.replace("*","")
    return s

def countDups(l):
    d={}
    for e in l:
        if e not in d:
            d[e]=_sage_const_1 
        else:
            d[e]+=_sage_const_1 
    
    return d

def __no_str_just_pol(ss,counter,sols):
    return ss 

def _str_just_pol(ss,counter):
    ### Conseguir soluciones!
    return ss

def _str_just_pol_no_sol(ss,counter):
    poly=ss[_sage_const_0 ]
    return "\\$P_{" + str(counter) + "}(x) = " + _latex(poly) + "$"


def str_just_pol(ss,counter,sols):
    if sols:
        return _str_just_pol(ss,counter)
    else:
        return _str_just_pol_no_sol(ss,counter)

def str_poly(ss,counter,sols):
    if sols:
        return _str_poly_sol(ss,counter)
    else:
        return _str_poly_no_sol(ss,counter)

def _str_poly_sol(ss,counter):    
    poly=ss[_sage_const_0 ]
    roots=""
    i=_sage_const_1 
    _rootDict=countDups(ss[_sage_const_1 ])
    for r in _rootDict:
        roots += "x_"+str(i)+" = " + _latex(r)
        if _rootDict[r] > _sage_const_1 :
            roots+= "["+ _latex(_rootDict[r]) + "]"
        roots+=" ; "
        i+=_sage_const_1 

    return "\\subitem $P_{" + str(counter) + "}(x) = " + _latex(poly) + "$ con raices: $" + _latex(roots) + " $"

def _str_poly_no_sol(ss,counter):    
    poly=ss

    return "\\subitem $P_{" + str(counter) + "}(x) = " + _latex(poly) + "$"


def mystr(s,funstr):
    return funstr(s)
    
def coprime(n):
    return [i for i in range(n) if gcd(i, n) == _sage_const_1  and i != _sage_const_1 ]
    
    
def random_between(mn,mx,integer):
    retval = random()*(mx-mn)+mn
    if integer:
        return int(retval)
    return retval

def _random_not_null(mn,mx,integer,reclim):
    retval = random_between(mn,mx,integer)
    if retval == _sage_const_0 :
        if mn == _sage_const_0  and (mx == _sage_const_1  or mx == _sage_const_0 ):
            return _sage_const_1 
        elif reclim > _sage_const_0 :
            return _random_not_null(mn,mx,integer,reclim-_sage_const_1 )
        else:
            return _sage_const_1 
    else:
        return retval


def random_not_null(mn,mx,integer):
    retval = random_between(mn,mx,integer)
    if retval == _sage_const_0 :
        if mn == _sage_const_0  and (mx == _sage_const_1  or mx == _sage_const_0 ):
            return _sage_const_1 
        else:
            return _random_not_null(mn,mx,integer,_sage_const_25 )
    else:
        return retval

def _genIrreductiblePoly():
    __tmp__=var("x"); P = symbolic_expression(x*x+random_not_null(_sage_const_0 ,_sage_const_5 ,true)).function(x)
    return P(x)

def __sum(a,b):
    return a+b
def __prod(a,b):
    return a*b




def _genP(grado,fixedroots,rfrac,rootsRank,coefRank,degree2):
    __tmp__=var("x"); P = symbolic_expression(_sage_const_1 ).function(x)
    raices=[]

    denAcum=_sage_const_1 
    
    for i in range(degree2):
        if degree2 and grado>_sage_const_2 :
            grado-=_sage_const_2 
            __tmp__=var("x"); P = symbolic_expression(P(x) * _genIrreductiblePoly()).function(x)

    for i in range(grado):
        if fixedroots == []:
            _r=random_not_null(rootsRank[_sage_const_0 ],rootsRank[_sage_const_1 ],true)
            den=_sage_const_1 
            

            if rfrac and _r!=_sage_const_0 :
                if _r == _sage_const_1  or _r == -_sage_const_1 :
                    den = random_between(_sage_const_2 ,_sage_const_4 ,true)
                else:
                    den = getDenominator(_r)
                rfrac-=_sage_const_1 
            raices.append(Rational(_sage_const_1p0 *_r/den))
            denAcum *= den
        else:
            raices.append(fixedroots.pop())

        __tmp__=var("x"); P = symbolic_expression(P(x) * (x-raices[i])).function(x)
    if denAcum == _sage_const_1 :
        coef=int(random_between(coefRank[_sage_const_0 ],coefRank[_sage_const_1 ],true))
        if coef == _sage_const_0 :
            coef=_sage_const_1 
    else:
        coef=denAcum
    return [coef*P(x),raices]
    

#Funcion para generar una cadena de texto a partir de un polinomio generado por la funcion auxiliar _genP,
#a partir de los argumentos recibidos.
def genP(grado,fixedroots,rfrac,printsol,strfun,counter,rootsRank,coefRank,degree2):
    _r=_genP(grado,fixedroots,rfrac,rootsRank,coefRank,degree2)    
    allPols.append(_r[_sage_const_0 ].expand())
    if printsol:
        return strfun([_r[_sage_const_0 ].expand(),_r[_sage_const_1 ]],counter,printsol)
    else:
        return strfun(_r[_sage_const_0 ].expand(),counter,printsol)



def _genPolNoRoots(grado,letters,printsol,strfun,coefRank,numTerms):
    return [reduce(__sum,[
                random_not_null(coefRank[_sage_const_0 ],coefRank[_sage_const_1 ],True)*
                reduce(__prod, [
                    l**random_not_null(_sage_const_1 ,ceil(grado/len(letters)),True) 
                for l in letters])
            for i in range(numTerms)]),""]

# Función para generar una cadena de texto a partir de un polinomio generado por la funcion auxiliar 
# _genPolNoRoots a partir de los argumentos recibidos.
def genPolNoRoots(grado,letters,printsol,strfun,coefRank,numTerms,counter):
    _r=_genPolNoRoots(grado,letters,printsol,strfun,coefRank,numTerms)    
    allPols.append(_r[_sage_const_0 ].expand())
    if printsol:
        return strfun([_r[_sage_const_0 ].expand(),_r[_sage_const_1 ]],counter,printsol)
    else:
        return strfun(_r[_sage_const_0 ].expand(),counter,printsol)

def identity(x):
    return x


def getDenominator(num):
    cps = coprime(num*num)
    den = cps[random_between(_sage_const_0 ,len(cps),true)]
    if den == _sage_const_0 :
        getDenominator(num)
    else:
        return den





# Escribimos el titulo. Recibe la informacion necesaria para describir el tipo de polinomios que viene a continuacion.
# int rfrac: Numero de raices fraccionarias
# int deg:   Grado de los polinomios.
# boolean degree2:  Si posee o no polinomios de segundo grado irreducibles.
def ppart(rfrac,deg,degree2):
    if rfrac:
        fracroots = "con hasta " + str(rfrac)  
    else:
        fracroots = "sin"

    deg2=""
    if degree2:
        deg2 = " (contiene un polinomio irreducible de grado 2)"
    
    return "" # "\\textbf{Polinomios de grado " + str(deg) + "} " #+ fracroots + " raices fraccionarias" + deg2 + ".\\\\\\"

# Devuelve el coeficniente principal del polinomio dado.
def getCoefPoly(poly):
    return [a[_sage_const_0 ] for a in poly.coefficients() if a[_sage_const_1 ] == poly.degree(x)][_sage_const_0 ]

# Construye el código látex de una función definida a trozos.
def construir_a_trozos(name,funs,fronts):
    retval = name + "=\\left\{\\begin{array}{ccc}"

    for i in range(len(funs)):
        if i == _sage_const_0 :
            _f = "x<"+str(fronts[i])
        elif i == (len(funs)-_sage_const_1 ):
            _f = "x>="+str(fronts[i-_sage_const_1 ])
        else:
            _f = str(fronts[i-_sage_const_1 ])+"<=x<"+str(fronts[i])
        retval += latex(funs[i]) + "& si & " + _f+"\\\\"
    retval+="\\end{array}\\right."
    return retval



def __getIntervalsFromIneq(tosolve):
    solutions = tosolve.solve(x)

    __aux = []
    for i in solutions:
        if type(i) == type(x+_sage_const_1 ):
            __aux.append(RealSet(i))
        elif type(i) == type([]):
            __aux.append(reduce(RealSet.intersection,[RealSet(a) for a in i]))
        else:
            __aux.append(RealSet(i[_sage_const_0 ]))

    if len(solutions) > _sage_const_1 :
        openpar = "\\left"
        closepar = "\\right"
    else:
        openpar = ""
        closepar = ""

    __intervals__ = " \\cup ".join(map(str,__aux))
    return openpar + "("+ __intervals__ + closepar + ")"



def genIdentidadNotable(tipo,fraccion,numletras,strfun):

    exp1 = random_not_null(_sage_const_2 ,_sage_const_7 ,true)
    if fraccion >= _sage_const_2 :
        den = getDenominator(exp1)
        exp1 = Rational(exp1*_sage_const_1p0 /den)
    letras_utilizadas=[]
    n=random_between(_sage_const_2 ,numletras,true)
    for b in range(n):
        letras_utilizadas.append(__letras[b])
        exp1 = exp1*__letras[b]
       


    exp2 = random_not_null(_sage_const_1 ,_sage_const_9 ,true)
    if fraccion >= _sage_const_1 :
        den = getDenominator(exp2)
        exp2 = Rational(exp2*_sage_const_1p0 /den)
    letras_para_usar = [item for item in __letras if item not in letras_utilizadas]
    for b in range(random_between(_sage_const_0 ,numletras - len(letras_utilizadas) ,true)):
        exp2 = exp2*letras_para_usar[b]


    if tipo == "suma":
        return "\\left(" + strfun(exp1) + "+" + strfun(exp2) + "\\right)^2"
    elif tipo == "resta":
        return "\\left(" + strfun(exp1) + "-" + strfun(exp2) + "\\right)^2"
    elif tipo == "sumaresta":
      return "\\left(" + strfun(exp1) + "+" + strfun(exp2) + "\\right)" + "\\left(" + strfun(exp1) + "-" + strfun(exp2) + "\\right)"
          


def GenerarListaCompletaSoluciones():
    rootsRank=[-_sage_const_3 ,_sage_const_4 ]            # Rango de valores que pueden tomar las raices
    coefRank=[-_sage_const_3 ,_sage_const_3 ]             # Rango de valores que pueden tomar los coeficientes principales.
    numToGen= _sage_const_6                  # Numero de polinomios a generar de cada tipo.
    printsol = false            # Imprimir las raices de cada polinomio.
    num_rfrac_max = _sage_const_5            # Numero maximo de raices fraccionarias.
    num_rfrac_min = _sage_const_2            # Numero minimo de raices fraccionarias.
    num=_sage_const_0                        # Contador auxiliar para llevar la numeracion de los polinomios generados.
    numDegree2Pols = _sage_const_1           # Numero maximo de polinomios irreducibles de grado 2.
    degree_max = _sage_const_5               # Grado maximo de los polinomios.
    degree_min = _sage_const_3               # Grado minimo de los polinomios.
    topMargin_solutions = _sage_const_1p2    # Margen vertical entre soluciones

    for _numDegree2Pols in range(numDegree2Pols+_sage_const_1 ):
        if _numDegree2Pols:
            print("\\section{Con " + str(_numDegree2Pols) + " polinomio/s irreducible/s de grado 2}")
        else:
            print("\\section{Sin polinomios irreducibles de grado 2}")

        for rfrac in range(num_rfrac_max-num_rfrac_min+_sage_const_1 ): # rfrac: numero de raices fraccionarias por polinomio.
            num_rfrac_real = rfrac + num_rfrac_min
            print("\\subsection{Hasta "+str(num_rfrac_real) + " raices fraccionarias}")
            for _deg in range(degree_max-degree_min+_sage_const_1 ): 
                deg=_deg+degree_min  # deg: grado del polinomio.
                if num_rfrac_real>deg:
                    num_rfrac_real=deg

                print(ppart(rfrac=num_rfrac_real,deg=deg,degree2=_numDegree2Pols))
                for i in xrange(numToGen): 
                    num = num + _sage_const_1 
                    print(genP(grado=deg,
                            fixedroots=[],
                            rfrac=num_rfrac_real,
                            printsol = printsol, 
                            strfun=str_poly, 
                            counter = num,
                            rootsRank = rootsRank,
                            coefRank = coefRank,
                            degree2 = _numDegree2Pols))
                    print("")

    print("\\newpage\\section{Soluciones}")
    for i in xrange(len(allPols)):
        p=allPols[i]
        print("\\subitem \\begin{dmath*}P_{"+str(i+_sage_const_1 )+"}(x) = " + latex(p.factor())+"\\end{dmath*}\\vspace{-" + str(topMargin_solutions) + "cm}")
    print("")

########################################## MAIN
allPols=[]

# Generar exámenes
rootsRank=[-_sage_const_3 ,_sage_const_4 ]            # Rango de valores que pueden tomar las raices
coefRank=[-_sage_const_4 ,_sage_const_4 ]             # Rango de valores que pueden tomar los coeficientes principales.
numToGen= _sage_const_6                  # Numero de polinomios a generar de cada tipo.
degree_max = _sage_const_5               # Grado maximo de los polinomios.
degree_min = _sage_const_3               # Grado minimo de los polinomios.
Enunciado=""
Solucion=""

genPolNoRoots(grado = degree_max,
            letters = [x,y,z],
            printsol = True,
            strfun = str_just_pol,
            coefRank = coefRank,
            numTerms = _sage_const_5 ,
            counter = _sage_const_0 )

#P1=genP(grado=3,fixedroots=[],rfrac=1,printsol = True, strfun=str_just_pol, counter = 0,rootsRank = rootsRank,coefRank = coefRank,degree2 = 0)
#P2=genP(grado=2,fixedroots=[],rfrac=1,printsol = True, strfun=str_just_pol, counter = 0,rootsRank = rootsRank,coefRank = coefRank,degree2 = 0)

#Enunciado ="\\paragraph{[4 puntos] 1) Dados $P(x) = "+latex(P1[0].expand())+"$ y $Q(x) = "+latex(P2[0].expand())+"$, realiza las siguientes operaciones:}"
#Enunciado += "\\begin{itemize}\\item\\textit{1 pto}\;\; $P(x) - Q(x)$\\item \\textit{1pto}\;\; $\\left(-2x^2\\right) \\cdot P(x)$\\item\\textit{2ptos}\;\;$P(x)\\cdot Q(x)$  \\end{itemize}"
#Enunciado += "\\paragraph{[6 puntos] 2) Resuelve las siguientes operaciones:}"
#Enunciado += "\\begin{itemize}"
#all_items = []
#all_items.append("\\item $" + genIdentidadNotable("suma",0,3,latex)+"$")
#all_items.append("\\item $" + genIdentidadNotable("suma",1,3,latex)+"$")
#all_items.append("\\item $" + genIdentidadNotable("resta",0,3,latex)+"$")
#all_items.append("\\item $" + genIdentidadNotable("resta",1,5,latex)+"$")
#all_items.append("\\item $" + genIdentidadNotable("sumaresta",0,1,latex)+"$")
#all_items.append("\\item $" + genIdentidadNotable("sumaresta",2,2,latex)+"$")
#shuffle(all_items)
#Enunciado += " ".join(all_items)
#Enunciado += "\\end{itemize}"


genPolNoRoots(grado = degree_max,
            letters = [x,y,z],
            printsol = True,
            strfun = str_just_pol,
            coefRank = coefRank,
            numTerms = _sage_const_5 ,
            counter = _sage_const_0 )



def genPolNoRoots_2024_05_02(letras,grado_max,nTerms,contador):
    return genPolNoRoots(grado = grado_max,
            letters = letras,
            printsol = True,
            strfun = __no_str_just_pol,
            coefRank = [-_sage_const_5 ,_sage_const_5 ],
            numTerms = nTerms,
            counter = contador)

Enunciado_2024_05_02 = "\\paragraph{ Realiza las siguientes operaciones }"
Solucion_2024_05_02 = ""





def _concat_with_plus(a,b):
    if a[_sage_const_0 ] == "-" and b[_sage_const_0 ] != "-":
        return b + " + " + a
    elif b[_sage_const_0 ] == "-" and a[_sage_const_0 ] == "-":
        return a + " + (" + b +")"
    else:
        return a + " + " + b

Enunciado_2024_05_02 += "\\begin{itemize}"
Solucion_2024_05_02 += "\\begin{itemize}"

num=_sage_const_0 

num += _sage_const_1 
Enunciado_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes sumas de polinomios: \\\\"
Solucion_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes sumas de polinomios: \\\\"

## EJERCICIO 1
## Solo suma de 3 polinomios de 3 letras
for a in range(_sage_const_10 ):
    grado_max = _sage_const_7 
    nTerms = _sage_const_3 
    letras = [x]
    P1 = genPolNoRoots_2024_05_02(letras,grado_max,nTerms,a)
    P2 = genPolNoRoots_2024_05_02(letras,grado_max,nTerms,a)
    P3 = genPolNoRoots_2024_05_02(letras,grado_max,nTerms,a)
    Enunciado_2024_05_02 += "\\noindent\\subitem ["+str(a+_sage_const_1 )+"]\\quad$" + reduce(_concat_with_plus,[latex(P1[_sage_const_0 ].expand()),latex(P2[_sage_const_0 ].expand()),latex(P3[_sage_const_0 ].expand())]) + "$"
    Solucion_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$" + reduce(_concat_with_plus,[latex(P1[_sage_const_0 ].expand()),latex(P2[_sage_const_0 ].expand()),latex(P3[_sage_const_0 ].expand())]) + " = " + latex((P1[_sage_const_0 ]+P2[_sage_const_0 ]+P3[_sage_const_0 ]).expand())+ "$"


## EJERCICIO 2
## Solo suma de 3 polinomios de 3 letras
num += _sage_const_1 
Enunciado_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes sumas de polinomios: \\\\"
Solucion_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes sumas de polinomios: \\\\"
for a in range(_sage_const_10 ):
    grado_max = _sage_const_7 
    nTerms = _sage_const_3 
    letras = [x,y,a]
    P1 = genPolNoRoots_2024_05_02(letras,grado_max,nTerms,a)
    P2 = genPolNoRoots_2024_05_02(letras,grado_max,nTerms,a)
    P3 = genPolNoRoots_2024_05_02(letras,grado_max,nTerms,a)
    Enunciado_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$" + reduce(_concat_with_plus,[latex(P1[_sage_const_0 ].expand()),latex(P2[_sage_const_0 ].expand()),latex(P3[_sage_const_0 ].expand())]) + "$"
    Solucion_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$" + reduce(_concat_with_plus,[latex(P1[_sage_const_0 ].expand()),latex(P2[_sage_const_0 ].expand()),latex(P3[_sage_const_0 ].expand())]) + " = " + latex((P1[_sage_const_0 ]+P2[_sage_const_0 ]+P3[_sage_const_0 ]).expand())+ "$"

## EJERCICIO 3
## Suma y resta de 3 polinomios de 3 letras
Enunciado_2024_05_02 += "\\item[Ejerciio 3] Realiza las siguientes sumas y restas de polinomios: \\\\"
Solucion_2024_05_02 += "\\item[Ejerciio 3] Realiza las siguientes sumas y restas de polinomios: \\\\"
for a in range(_sage_const_10 ):
    grado_max = _sage_const_7 
    nTerms = _sage_const_3 
    letras = [x,y,a]
    P1 = genPolNoRoots_2024_05_02(letras,grado_max,nTerms,a)
    P2 = genPolNoRoots_2024_05_02(letras,grado_max,nTerms,a)
    P3 = genPolNoRoots_2024_05_02(letras,grado_max,nTerms,a)
    if a%_sage_const_3 :
        Enunciado_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$" + latex(P1[_sage_const_0 ].expand())+" + "+latex(P2[_sage_const_0 ].expand())+ "- (" + latex(P3[_sage_const_0 ].expand())+")" + "$"
        Solucion_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$" + latex(P1[_sage_const_0 ].expand())+" + "+latex(P2[_sage_const_0 ].expand())+ "- (" + latex(P3[_sage_const_0 ].expand())+")" + " = " + latex((P1[_sage_const_0 ]+P2[_sage_const_0 ]-P3[_sage_const_0 ]).expand())+ "$"
    elif (a+_sage_const_1 )%_sage_const_3 :
        Enunciado_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$" + latex(P1[_sage_const_0 ].expand())+" - ("+latex(P2[_sage_const_0 ].expand())+ ") + (" + latex(P3[_sage_const_0 ].expand())+")" + "$"
        Solucion_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$" + latex(P1[_sage_const_0 ].expand())+" - ("+latex(P2[_sage_const_0 ].expand())+ ") + (" + latex(P3[_sage_const_0 ].expand())+")" + " = " + latex((P1[_sage_const_0 ]-P2[_sage_const_0 ]+P3[_sage_const_0 ]).expand())+ "$"
    else:
        Enunciado_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$" + latex(P1[_sage_const_0 ].expand())+" - ("+latex(P2[_sage_const_0 ].expand())+ ") - (" + latex(P3[_sage_const_0 ].expand())+")" + "$"
        Solucion_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$" + latex(P1[_sage_const_0 ].expand())+" - ("+latex(P2[_sage_const_0 ].expand())+ ") - (" + latex(P3[_sage_const_0 ].expand())+")" + " = " + latex((P1[_sage_const_0 ]-P2[_sage_const_0 ]-P3[_sage_const_0 ]).expand())+ "$"
    

    

## EJERCICIO 4
## Multiplicación de monomios
num += _sage_const_1 
Enunciado_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes multiplicaciones de monomios: \\\\"
Solucion_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes multiplicaciones de monomios: \\\\"

for a in range(_sage_const_10 ):
    grado_max = _sage_const_3 *_sage_const_5 +_sage_const_1 
    nTerms = _sage_const_1 
    letras = [x,y,z,a,b]
    P1 = genPolNoRoots_2024_05_02(letras,grado_max,nTerms,a)
    P2 = genPolNoRoots_2024_05_02(letras,grado_max,nTerms,a)
    P3 = genPolNoRoots_2024_05_02(letras,grado_max,nTerms,a)
    Enunciado_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$ (" + latex(P1[_sage_const_0 ].expand())+") · ("+latex(P2[_sage_const_0 ].expand())+ ")$"
    Solucion_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$ (" + latex(P1[_sage_const_0 ].expand())+") · ("+latex(P2[_sage_const_0 ].expand())+ ") = " + latex((P1[_sage_const_0 ]*P2[_sage_const_0 ]).expand())+ "$"




## EJERCICIO 5
## Multiplicación de monomio por polinomio
num += _sage_const_1 
Enunciado_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes multiplicaciones de polinomios: \\\\"
Solucion_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes multiplicaciones de polinomios: \\\\"

for a in range(_sage_const_10 ):
    grado_max = _sage_const_3 
    nTerms = _sage_const_3 
    letras = [x]
    P1 = genPolNoRoots_2024_05_02(letras,grado_max,_sage_const_1 ,a)
    P2 = genPolNoRoots_2024_05_02(letras,grado_max,_sage_const_4 ,a)
    Enunciado_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$ (" + latex(P1[_sage_const_0 ].expand())+") · ("+latex(P2[_sage_const_0 ].expand())+ ")$"
    Solucion_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$ (" + latex(P1[_sage_const_0 ].expand())+") · ("+latex(P2[_sage_const_0 ].expand())+ ") = " + latex((P1[_sage_const_0 ]*P2[_sage_const_0 ]).expand())+ "$"


## EJERCICIO 5
## Multiplicación de polinomios fácil
num += _sage_const_1 
Enunciado_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes multiplicaciones de polinomios: \\\\"
Solucion_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes multiplicaciones de polinomios: \\\\"

for a in range(_sage_const_15 ):
    grado_max = _sage_const_3 
    nTerms = _sage_const_3 
    letras = [x]
    P1 = genPolNoRoots_2024_05_02(letras,grado_max,_sage_const_2 ,a)
    P2 = genPolNoRoots_2024_05_02(letras,grado_max,_sage_const_3 ,a)
    Enunciado_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$ (" + latex(P1[_sage_const_0 ].expand())+") · ("+latex(P2[_sage_const_0 ].expand())+ ")$"
    Solucion_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$ (" + latex(P1[_sage_const_0 ].expand())+") · ("+latex(P2[_sage_const_0 ].expand())+ ") = " + latex((P1[_sage_const_0 ]*P2[_sage_const_0 ]).expand())+ "$"



## EJERCICIO 6
## Multiplicación de polinomios
num += _sage_const_1 
Enunciado_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes multiplicaciones de polinomios: \\\\"
Solucion_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes multiplicaciones de polinomios: \\\\"

for a in range(_sage_const_20 ):
    grado_max = _sage_const_4 
    nTerms = _sage_const_3 
    letras = [x]
    if a%_sage_const_3 :
        P1 = genPolNoRoots_2024_05_02(letras,grado_max,_sage_const_2 ,a)
        P2 = genPolNoRoots_2024_05_02(letras,grado_max,_sage_const_4 ,a)
    elif (a+_sage_const_1 )%_sage_const_3 :
        P1 = genPolNoRoots_2024_05_02(letras,grado_max,_sage_const_3 ,a)
        P2 = genPolNoRoots_2024_05_02(letras,grado_max,_sage_const_3 ,a)
    else:
        P1 = genPolNoRoots_2024_05_02(letras,grado_max,_sage_const_3 ,a)
        P2 = genPolNoRoots_2024_05_02(letras,grado_max,_sage_const_2 ,a)

    Enunciado_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$ (" + latex(P1[_sage_const_0 ].expand())+") · ("+latex(P2[_sage_const_0 ].expand())+ ")$"
    Solucion_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$ (" + latex(P1[_sage_const_0 ].expand())+") · ("+latex(P2[_sage_const_0 ].expand())+ ") = " + latex((P1[_sage_const_0 ]*P2[_sage_const_0 ]).expand())+ "$"


## EJERCICIO 7
## Multiplicación de polinomios
num += _sage_const_1 
Enunciado_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes multiplicaciones de polinomios: \\\\"
Solucion_2024_05_02 += "\\item[Ejercicio " + str(num) +":] Realiza las siguientes multiplicaciones de polinomios: \\\\"

for a in range(_sage_const_7 ):
    grado_max = _sage_const_5 
    nTerms = _sage_const_4 
    letras = [x,y]
    P1 = genPolNoRoots_2024_05_02(letras,grado_max,_sage_const_2 ,a)
    P2 = genPolNoRoots_2024_05_02(letras,grado_max,_sage_const_3 ,a)
    Enunciado_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$ (" + latex(P1[_sage_const_0 ].expand())+") · ("+latex(P2[_sage_const_0 ].expand())+ ")$"
    Solucion_2024_05_02 += "\\subitem ["+str(a+_sage_const_1 )+"]\\quad$ (" + latex(P1[_sage_const_0 ].expand())+") · ("+latex(P2[_sage_const_0 ].expand())+ ") = " + latex((P1[_sage_const_0 ]*P2[_sage_const_0 ]).expand())+ "$"

Enunciado_2024_05_02 += "\\end{itemize}\n\n"
Solucion_2024_05_02 += "\\end{itemize}\n\n"

print(Enunciado_2024_05_02)
print(Solucion_2024_05_02)


f=open("polys.tex","w")
f.write(Enunciado_2024_05_02)
f.close()
g=open("polysSol.tex","w")
g.write(Solucion_2024_05_02)
g.close()

